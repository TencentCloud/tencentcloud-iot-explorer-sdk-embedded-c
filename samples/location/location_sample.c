...
static void electric_fence_aleart_cb(void *pContext, char *msg, uint32_t msgLen)
{
    Log_w("electric fence aleart msg: %s", msg);
    char *client_token = LITE_json_value_of("clientToken", msg);
    if (client_token) {
        IOT_Location_Fence_Aleart_Reply(IOT_Template_Get_MQTT_Client(pContext), client_token, 0, "success");
        HAL_Free(client_token);
    } else {
        Log_e("parse alert fence event error.");
    }
}
...
int main(int argc, char **argv)
{
    ...
    IOT_Location_Capture_Fence_Aleart_Register(IOT_Template_Get_MQTT_Client(client), electric_fence_aleart_cb, client);
