...
#define MAX_ARRAY_ITEM_STR_LEN (64)
#else
...
static int  sg_alias_name_array_size = 0;
static char sg_alias_name_array[MAX_SAMPLE_ARRAY_SIZE][64];

static int sg_power_consumption_array_size = 0;
static int sg_power_consumption_array[MAX_SAMPLE_ARRAY_SIZE];

static int             sg_recent_status_array_size = 0;
static recent_status_t sg_recent_status_array[MAX_SAMPLE_ARRAY_SIZE];
#endif
...
static void _init_data_template(void)
{
...
#if TEST_ARRAY
    memset(sg_ProductData.m_alias_name, 0, sizeof(sg_ProductData.m_alias_name));
    sg_DataTemplate[5].data_property.data                  = sg_ProductData.m_alias_name;
    sg_DataTemplate[5].data_property.array_info.array_size = MAX_ARRAY_JSON_STR_LEN;
    sg_DataTemplate[5].data_property.array_info.array_type = TYPE_TEMPLATE_STRING;
    ...
    sg_DataTemplate[6].data_property.array_info.array_type = TYPE_TEMPLATE_INT;
    ...
    sg_DataTemplate[7].data_property.array_info.array_type = TYPE_TEMPLATE_JOBJECT;
    sg_DataTemplate[7].data_property.key                   = "recent_status";
    sg_DataTemplate[7].data_property.type                  = TYPE_TEMPLATE_ARRAY;
...
static void _refresh_recent_status()
{
    ...
    for (int ii = 0; ii < sg_recent_status_array_size; ++ii) {
        obj_strs[ii] = HAL_Malloc(MAX_ARRAY_ITEM_STR_LEN);
        ret          = HAL_Snprintf(obj_strs[ii], MAX_ARRAY_ITEM_STR_LEN - 1, "{\"brightness\":%d, \"temperature\":%f}",
...
{
    int ret = 0;
    if (obj_len != sizeof(recent_status_t)) {
        Log_e("obj_len %zu != %zu", obj_len, sizeof(recent_status_t));

        return QCLOUD_ERR_FAILURE;
    }
    recent_status_t *p_rs = (recent_status_t *)obj;
    ...
    if (!(p = LITE_json_value_of("brightness", temp)))
        return QCLOUD_ERR_FAILURE;
    ret = LITE_get_int32(&(p_rs->brightness), p);
    HAL_Free(p);
    if (ret < 0)
        return ret;

    if (!(p = LITE_json_value_of("temperature", temp)))
        return QCLOUD_ERR_FAILURE;
    ret = LITE_get_float(&(p_rs->temperature), p);
    HAL_Free(p);

    return ret;
}

/* do with the array information received */
static int _on_recv_array_msg(DeviceProperty *p_prop)
{
    int res = 0;
    if (p_prop->type != JARRAY)
        return 0;
    const char *key = p_prop->key;
    Log_d("key : [%s] data : [%s] ", p_prop->key, p_prop->data);
