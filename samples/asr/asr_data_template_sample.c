...
static void _init_data_template(void)
{
    ...
    sg_DataTemplate[0].data_property.key  = "asr_response";
...
{
	Log_d("request_id:%d: %d/%d %s", request_id, resutl_seq, total_resutl_num, res_text);
}
...
int main(int argc, char **argv)
{
	...
	asr_client = IOT_Asr_Init(devInfo.product_id, devInfo.device_name, IOT_Template_Get_MQTT_Client(data_template_client), NULL);
    if (asr_client == NULL) {
    ...
    while (IOT_Template_IsConnected(data_template_client) || rc == QCLOUD_ERR_MQTT_ATTEMPTING_RECONNECT ||
		...
		//asr record file requst, sg_ProductData.m_res_text from OnControlMsgCallback is asr result
		RecordAsrConf config = {0};
		rc = IOT_Asr_RecordFile_Request(asr_client, "./test_file/test.wav", &config, asr_result_cb);
		if(rc > 0){
			Log_d("record file %s's request_id %d ", "test.wav", rc);
		}else{
