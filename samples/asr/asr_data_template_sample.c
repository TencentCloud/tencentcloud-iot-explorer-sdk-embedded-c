...
typedef struct _ProductDataDefine {
    TYPE_DEF_TEMPLATE_STRING m_asr_response[2048+1];
} ProductDataDefine;
...
{
    sg_ProductData.m_asr_response[0] = '\0';
    sg_DataTemplate[0].data_property.data = sg_ProductData.m_asr_response;
    sg_DataTemplate[0].data_property.data_buff_len = sizeof(sg_ProductData.m_asr_response)/sizeof(sg_ProductData.m_asr_response[0]);
    sg_DataTemplate[0].data_property.key  = ASR_RESPONSE_PROPERTY_KEY;
...
static void OnControlMsgCallback(void *pClient, const char *pJsonValueBuffer, uint32_t valueLength, DeviceProperty *pProperty)
{
    ...
    for (i = 0; i < TOTAL_PROPERTY_COUNT; i++) {
        ...
        if (strcmp(sg_DataTemplate[i].data_property.key, pProperty->key) == 0) {
            ...
            if(!strcmp(pProperty->key, ASR_RESPONSE_PROPERTY_KEY) && (NULL != sg_asr_client)) {
                LITE_string_strip_char((char *)pProperty->data, '\\');
                int rc = IOT_Asr_Result_Notify(sg_asr_client, (char *)pProperty->data);
...
static void deal_down_stream_user_logic(void *client, ProductDataDefine *asr)
{
    ...
    if(eCHANGED == get_property_state(asr->m_asr_response)) {
        set_property_state(asr->m_asr_response, eNOCHANGE);
    }
}
...
int main(int argc, char **argv)
{
    ...
    while (IOT_Template_IsConnected(data_template_client) || rc == QCLOUD_ERR_MQTT_ATTEMPTING_RECONNECT ||
        ...
        if(DEMO_ASR == DEMO_ASR_FILE) {
            ...
            rc = IOT_Asr_RecordFile_Request(sg_asr_client, "./test_file/test.wav", &config, asr_result_cb);
            if(rc > 0) {
        ...
        if(DEMO_ASR == DEMO_ASR_REALTIEM) {
            ...
            while(!HAL_FileEof(fp) && file_size > 0) {
                ...
                rc = IOT_Asr_Realtime_Request(sg_asr_client, audio_buff, read_len, &conf, asr_result_cb);
                if(rc > 0) {
...
exit:
    ...
    rc |=  IOT_Asr_Destroy(sg_asr_client);
