...
#define NETWORK_ADDR_LEN (16)
    ...
    for (cur = addr_list; cur != NULL; cur = cur->ai_next) {
        ...
        Log_d("establish udpconnection with server(host=%d port=%d)", ((struct sockaddr_in *)cur->ai_addr)->sin_addr,
...
int HAL_UDP_WriteTo(uintptr_t fd, const unsigned char *p_data, unsigned int datalen, char *host, unsigned short port)
{
    ...
    if ((rc = getaddrinfo(host, port_str, &hints, &addr_list)) != 0) {
        Log_e("getaddrinfo error,errno:%s, rc:%d, %s", strerror(errno), rc, gai_strerror(rc));
        return 0;
    }
