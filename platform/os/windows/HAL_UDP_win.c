...
#define NETWORK_ADDR_LEN (16)
    ...
    if ((ret = getaddrinfo(host, port_str, &hints, &addr_list)) != 0) {
        Log_e("getaddrinfo error,errno:%s, ret:%d", strerror(errno), ret);
        return -1;
    }
    ...
    Log_i("HAL_UDP_CreateBind fd:%d", ret);
                                ...
                                char *recv_ip_addr, unsigned char recv_addr_len, unsigned short *recv_port)
{
    ...
    /* Zero fds ready means we timed out */
    if (ret == 0) {
        Log_i("HAL_UDP_ReadTimeoutPeerInfo Timeout");
        return 0; /* receive timeout */
    }

    if (ret < 0) {
        Log_i("HAL_UDP_ReadTimeoutPeerInfo ret < 0 :%d", ret);
        if (errno == EINTR) {
            return -3; /* want read */
        }
    ...
    Log_i("HAL_UDP_ReadTimeoutPeerInfo SELECT SUCCSS fd:%d", fd);

    /* This call will not block */
    len = recvfrom((int)fd, p_data, datalen, 0, (struct sockaddr *)&source_addr, &addrLen);

    memset(recv_ip_addr, 0, recv_addr_len);
    strcpy(recv_ip_addr, inet_ntoa(((struct sockaddr_in *)&source_addr)->sin_addr));
...
int HAL_UDP_WriteTo(uintptr_t fd, const unsigned char *p_data, unsigned int datalen, char *host, unsigned short port)
{
    ...
    rc = sendto(socket_id, p_data, datalen, 0, addr_list->ai_addr, addr_list->ai_addrlen);
