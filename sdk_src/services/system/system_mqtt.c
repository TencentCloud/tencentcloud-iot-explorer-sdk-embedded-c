...
static SysMQTTState sg_state = {.topic_sub_ok = false, .result_recv_ok = false, .time = 0};
...
static int _iot_system_info_get_publish(void *pClient, DeviceInfo *dev_info)
{
...
int IOT_Get_SysTime(void *pClient, long *time, DeviceInfo *dev_info)
{
    int ret    = 0;
    int cntSub = 0;
    int cntRev = 0;
    ...
    if (!sg_state.topic_sub_ok) {
        for (cntSub = 0; cntSub < 3; cntSub++) {
            ret = _iot_system_info_result_subscribe(mqtt_client, dev_info);
            if (ret < 0) {
    ...
    ret = _iot_system_info_get_publish(mqtt_client, dev_info);
    if (ret < 0) {
